...
object Sevm extends Requirements {
  ...
  val arithLexers = mkLexers0(0x01)(ADD, MUL, SUB, DIV, SDIV, MOD, SMOD, ADDMOD, MULMOD, EXP, SIGNEXTEND)
  ...
  val compLexers = mkLexers0(0x10)(LT, GT, SLT, SGT, EQ, ISZERO, AND, OR, XOR, NOT, BYTE)

  case object SHA3 extends Op
  val sha3Lexer = mkLexers0(0x20)(SHA3)
  ...
  val blockLexers = mkLexers0(0x40)(BLOCKHASH, COINBASE, TIMESTAMP, NUMBER, DIFFICULTY, GASLIMIT)
  ...
  val stackMemStorageFlow = mkLexers0(0x50)(
  ...
  val allLexers =
    arithLexers ++ compLexers ++ compLexers ++ sha3Lexer ++ envLexers ++
  ...
  def lex(lexer: Lexer, lexers: Lexer*)(source: Source): Disassembly Or (Disassembly, ErrorMessage) = {
    val combinedLexer = lexers.foldLeft(lexer)(_ orElse _).lift
